...
/**
 * Verify OTP for an email (but don't delete it yet)
 */
export async function verifyOTP(email: string, inputOTP: string): Promise<{ 
  ...
  try {
    ...
    if (otpData.otp === inputOTP) {
      // Mark as verified but don't delete yet - we'll delete it after successful registration
      await prisma.$executeRaw`
        UPDATE otp_verifications 
        SET attempts = -1, "updatedAt" = NOW()
        WHERE email = ${normalizedEmail}
      `
      return {
        success: true,
...
export async function isOTPVerified(email: string): Promise<boolean> {
  try {
    ...
    const otpData = result[0]
    return otpData.attempts === -1 && Date.now() <= otpData.expiresAt.getTime()
  } catch (error) {
    console.error('Check OTP verified error:', error)
    return false
  }
}
...
export async function deleteOTP(email: string): Promise<void> {
  try {
    await prisma.$executeRaw`DELETE FROM otp_verifications WHERE email = ${email.toLowerCase()}`
  } catch (error) {
